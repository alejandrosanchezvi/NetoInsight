// 📊 NetoInsight - Ordenes-Compra Component (EMBEDDING API v3 - OPTIMIZADO)
...
export class Ordenescompra implements OnInit, AfterViewInit, OnDestroy {
  ...
  private adjustForSidebar(): void {
    const container = document.querySelector('.ordenes-compra-container') as HTMLElement;
    if (container) {
  ...
  private async loadDashboard(): Promise<void> {
    try {
      ...
      const response = await this.http.get<any>(
        ...
        {
          params: { dashboard: 'purchase-orders' },
