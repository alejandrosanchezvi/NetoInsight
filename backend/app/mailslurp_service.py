...
class MailSlurpService:
    ...
    def __init__(self):
        ...
        self.inbox_id   = os.getenv('MAILSLURP_INBOX_ID', '81ab8878-8987-4a9c-a534-299ef25dbe2f')
        self.from_email = os.getenv('MAILSLURP_FROM_EMAIL', '81ab8878-8987-4a9c-a534-299ef25dbe2f@soyneto.com')
    ...
    def send_invitation_email(
        ...
        try:
            ...
            return {"success": True, "message_id": result.id,
                    "sent_at": datetime.utcnow().isoformat(), "recipient": email}
    ...
    def verify_connection(self) -> Dict:
        try:
            ...
            return {"success": True, "inbox_id": self.inbox_id,
    ...
    def _build_invitation_html(
        self, email, invitation_token, tenant_name,
        invited_by_name, invited_by_email, role, expires_at, frontend_url
    ) -> str:
        html = self._load_template('invitation_email.html') or self._fallback_invitation_html()
    ...
    def _build_password_reset_html(
        ...
        html = self._load_template('password_reset_email.html') or self._fallback_reset_html()

        replacements = {
            '{{email}}':          email,
            '{{resetUrl}}':       reset_url,
            '{{requestDate}}':    datetime.utcnow().strftime("%d de %B de %Y a las %H:%M UTC"),
            '{{expirationDate}}': expires_at.strftime("%d de %B de %Y a las %H:%M UTC"),
            '{{currentYear}}':    str(datetime.now().year),
        }
        for k, v in replacements.items():
