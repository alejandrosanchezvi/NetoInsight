...
if not firebase_admin._apps:
    try:
        firebase_admin.initialize_app(
            ...
            options={"projectId": os.getenv("FIREBASE_PROJECT_ID", "netoinsight-fed03")}
        )
        logging.getLogger(__name__).info("✅ [FIREBASE-ADMIN] Initialized — project: netoinsight-fed03")
...
def read_root():
    return {
        "status": "ok", "service": "NetoInsight API", "version": "2.5.0",
...
def health_check():
    return {
        "status": "healthy", "api_version": "v2.5",
...
async def send_password_reset(request: PasswordResetRequest):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"🔒 [RESET] User not found (generic response): {request.email}")
            return GENERIC_RESPONSE
        except Exception as e:
            logger.error(f"❌ [RESET] get_user_by_email failed — {type(e).__name__}: {str(e)}")
            raise HTTPException(status_code=500, detail="Error verificando usuario")

        action_code_settings = firebase_auth.ActionCodeSettings(
            url=f"{request.frontend_url}/reset-password",
        ...
        try:
            ...
            logger.error(f"❌ [RESET] generate_password_reset_link failed — {type(e).__name__}: {str(e)}")
            raise HTTPException(status_code=500, detail="Error generando enlace de reset")
...
async def verify_mailslurp_connection(user_data: dict = Depends(verify_token)):
    try:
        ...
        if result["success"]:
            return {"success": True, "inbox_id": result["inbox_id"],
                    "email_address": result["email_address"]}
        raise HTTPException(status_code=500, detail=f"MailSlurp connection failed: {result.get('error')}")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_embed_url_v3(dashboard: str, user_data: dict = Depends(verify_token)):
    ...
    try:
        ...
        tenant_name = user_data.get('tenant_name') or user_data.get('tenantName') or 'Unknown'
...
def list_available_dashboards():
    names = {
        'categorization': 'Categorización', 'stores': 'Tiendas',
        'skus': 'SKUs', 'stocks': 'Stocks', 'purchase-orders': 'Órdenes de Compra'
    }
    return {"dashboards": [{"key": k, "name": names[k], "path": p}
                           for k, p in DASHBOARD_MAP.items()]}